...
// 获取今年的阳历生日日期
function getThisYearBirthday(birthday: Birthday): { month: number; day: number; year: number } {
  const today = new Date();
  const currentYear = today.getFullYear();

  const thisYearDate = new Date(currentYear, birthday.solarMonth - 1, birthday.solarDay);

  if (thisYearDate >= new Date(currentYear, today.getMonth(), today.getDate())) {
    return { month: birthday.solarMonth, day: birthday.solarDay, year: currentYear };
  } else {
    return { month: birthday.solarMonth, day: birthday.solarDay, year: currentYear + 1 };
  }
}
                  ...
                  <div className="flex items-center justify-between text-sm">
                    ...
                    <span className="text-gray-400">
                      {(() => {
                        ...
                        return `${thisYearBirthday.year}年${thisYearBirthday.month}月${thisYearBirthday.day}日`;
                      })()}
                    </span>
                  </div>
                </div>
